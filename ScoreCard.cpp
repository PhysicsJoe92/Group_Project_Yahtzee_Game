...
void ScoreCard::saveCard(string name){
    ofstream out("saves/"+name+".dat", ios::out | ios::binary);
    if (!out) {
        cerr << "Error: failed to open " << name << " for writing." << endl;
        return;
    }
    // Save current game
...
void ScoreCard::loadCard(string name){
    ifstream in("saves/"+name+".dat", ios::in | ios::binary);
    if (!in) {
        cerr << "Error: failed to open " << name << " for reading." << endl;
        return;
    }
    // Load current game
...
void ScoreCard::replaceCard(string name){
    string filename = "saves/" + name + ".dat";
    ...
    // If new game will be the 6th game, erase last score card and replace with default
    if(currGame < 5){
        // Read upper section
